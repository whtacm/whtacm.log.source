...
通过打印 `numpy` 的路径，发现这个路径不是我下载的位置（我通过 `pip install` 下载的位置是 `/Library/Python/2.7/site-packages/`），然后继续运行之情的 `Python` 命令运行项目，发现 `numpy` 这个不报错了，但是有了新的问题，定位到这个一句 ` type object 'NewBase' has no attribute 'is_abstract'`，找不到的错误。
